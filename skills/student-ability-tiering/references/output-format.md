...
**`next_session.exclude_qids` 必须由调用方合并回画像，否则下一轮推出同一批题。** 本 skill 只读画像不回写（见 `profile-contract.md` 第九节），这份合并是调用方的职责，链路上没有别的环节兜。判定说明里点明这一条，不要指望别人自己发现。孩子做完之后走报告环节的话，这份 `exclude_qids` 会原样进报告的 `profile_patch.exclude_qids`，合并动作仍然归调用方，见 `report-format.md`。
...
出题时报一次是让学生翻得到，**讲完再报一次是让他标得上**——错题要回到书上那一道去圈，隔一周复习时找得回来。所以做对、做错两个场景的话术末尾都带一句出处，收尾时再把本次覆盖的范围归拢成一句。

出处怎么说取决于 `items[].source`：

| `source` | 说法 | 依据字段 |
| --- | --- | --- |
| `workbook` | 「这道是《53同步》Unit 1 第 3 课时第 7 题，在书上标一下」 | `locator` 的 `unit` + `period` + `question_no` |
| `variant` | 「这道是我照着 Unit 1 第 3 课时第 7 题改的，考点一样、换了个场景」 | `derived_from` 同上三项 + `rule` |
| `generated` | 「这道书上没有，是照着 by 后面跟动名词这个考点出的」 | `knowledge_point`，**不提课时题号** |

`exercise_no` 是罗马数字的练习序号，说出口反而费解，**话术里不说**，只说单元、课时、题号；`question_no` 在课时内连续，学生按这个号能直接找到。这三项以外的定位（页码、章节名）题库里根本没有，见 `bank-contract.md` 第六节。

收尾那句归拢到单元与课时即可：「今天做的都在 Unit 1 第 3 课时，错的那两道我标了，明天先过一遍」。
