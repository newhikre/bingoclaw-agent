...
本 skill 的唯一必需输入是前置 skill `build-learner-profile` 的 `evaluate` 输出，下称**画像**。

**本文件是画像的完整定义，读这一份就够。** 运行时不要求能访问前置 skill 的任何文件——包括它的 `profile-schema.md`。开发环境下两者应保持一致，出现分歧时以本文件描述的实际行为为准，并在 `data_gaps` 里记一条。

---

## 一、上游如何产出画像

前置 skill 提供一个脚本 `scripts/profile_engine.py`，三个子命令：
...
三个命令**都只往 stdout 写 JSON**（`indent=2`、`ensure_ascii=False`），**不生成任何文件**。落盘是调用方的动作，不是上游行为。
...
| 文件 | 内容 | 谁写 |
| --- | --- | --- |
| `session.json` | 诊断会话，学生的原始作答 | 前置 skill 的调用方 |
| `profile.json` | `evaluate` 的 stdout 重定向结果，即画像 | 前置 skill 的调用方 |
| `task_pack.json` | 本 skill 的任务包 | 本 skill |
...
| 10 | 顶层是 `{"ok": false, "error": ...}` | 上游执行失败 | **不是画像**。停止，把 error 原样报给调用方，不要凭空造画像 |
| 11 | 缺 `strategy.code` | 传入的不是 evaluate 输出 | **唯一的硬失败。** 拒绝继续，要求调用方补跑 `evaluate`。本 skill 不自行判层——两段链路给出互相矛盾的层级，比不出结果更糟 |
...
**只读不写。** 本 skill 不修改画像、不回写字段。要交给下一环节的东西（例如本次消耗掉的题）放在任务包的 `next_session.exclude_qids` 里，由调用方决定怎么合并。
