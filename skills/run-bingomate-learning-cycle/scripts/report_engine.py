...
def locator_key(loc: Any) -> str | None:
    """Stable internal provenance key; never used as the response join key."""
    if isinstance(loc, str):
        key = "/".join(part.strip() for part in loc.split("/") if part.strip())
        return key or None
...
def append_round(path: Path, pack: dict | None, log: dict | None, round_no: int | None) -> dict:
    ...
    if pack is not None:
        ...
        if pack.get("session_id") not in (None, data["session_id"]):
            raise ValueError("任务包 session_id 与会话文件不一致")
        # The report is built from task packs, so the canonical session id must
        # be written back into every pack rather than living only on the wrapper.
        pack["session_id"] = data["session_id"]
...
def grade_item(item: dict, record: dict | None) -> dict:
    ...
    if isinstance(spec, dict):  # 篇章材料：{"8": [...], "9": [...]}
        ...
        for no, sub_spec in spec.items():
            ...
            attempted = student not in (None, "", {}, [])
            points.append(
                {
                    "question_no": no,
                    "response": student,
                    "attempted": attempted,
                    "correct": attempted and is_correct(student, sub_spec),
                }
            )
    else:
        points.append(
    ...
    total = sum(1 for p in points if p["attempted"])
    right = sum(1 for p in points if p["attempted"] and p["correct"])
    answered = total > 0
    if not answered:
        status = "未作答"
    elif right == total:
...
def build_round(pack: dict, log: dict, index: int) -> dict:
    ...
    item_seqs = [item.get("seq") for item in items]
    if None in item_seqs or len(item_seqs) != len(set(item_seqs)):
        raise ValueError(f"第 {index} 轮：任务包 seq 缺失或重复")
    item_ids = [item.get("item_id") for item in items if item.get("item_id")]
    if len(item_ids) != len(set(item_ids)):
        raise ValueError(f"第 {index} 轮：任务包 item_id 重复")
    response_rows = log.get("responses", [])
    if not isinstance(response_rows, list):
        raise ValueError(f"第 {index} 轮：作答记录 responses 必须是数组")
    response_seqs = [r.get("seq") for r in response_rows if isinstance(r, dict)]
    if len(response_seqs) != len(set(response_seqs)):
        raise ValueError(f"第 {index} 轮：作答记录 seq 重复")
    records = {r.get("seq"): r for r in response_rows}
    unknown = sorted(str(s) for s in records if s not in set(item_seqs))
    if unknown:
    ...
    for seq, record in records.items():
        expected = by_seq[seq].get("item_id")
        supplied = record.get("item_id")
        if expected and supplied and expected != supplied:
            raise ValueError(f"第 {index} 轮：seq {seq} 的 item_id 与任务包不一致")
    ...
    return {
        ...
        "current_used_item_ids": [
            item.get("item_id")
            or item.get("source_qid")
            or item.get("qid")
            or locator_key(item.get("locator"))
            for item in items
        ],
        "data_gaps": gaps,
    }
...
def build_report(
    ...
    current_used: list[str] = []
    for r in rounds:
        for item_id in r["current_used_item_ids"]:
            if item_id and item_id not in current_used:
                current_used.append(str(item_id))

    pattern = error_pattern(part_rates, graded)
    return {
        "report_id": f"rep-{datetime.now().strftime('%Y%m%d-%H%M%S')}",
        "session_id": next((r["session_id"] for r in rounds if r["session_id"]), None),
        ...
        "profile_patch": {
            ...
            "cumulative_exclude_item_ids": excluded,
            "current_used_item_ids": current_used,
            ...
            "error_pattern": pattern,
            "apply_note": "由 cycle_engine 确定性合并回画像；策略代码保持不变",
        },
        "data_gaps": data_gaps,
        "out_of_scope": ["层级判定与升降层（需重新摸底）", "跨会话趋势与遗忘曲线复习调度"],
    }
...
def selfcheck() -> dict:
    ...
    # 篇章只答一部分时，未答小题不进入正确率分母。
    partial_passage = grade_item(
        pack["items"][3],
        {"seq": 4, "response": {"8": "B"}, "hints_used": 0},
    )
    assert partial_passage["points_planned"] == 3
    assert partial_passage["points_total"] == 1
    assert partial_passage["points_correct"] == 1
    assert partial_passage["status"] == "正确"
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        # 自动生成的 session_id 必须同步写入 task_pack，报告不能再变成 null。
        anonymous = json.loads(json.dumps(pack, ensure_ascii=False))
        anonymous.pop("session_id", None)
        anonymous_path = Path(tmp) / "anonymous-session.json"
        appended = append_round(anonymous_path, anonymous, None, None)
        append_round(anonymous_path, None, log, None)
        anon_pairs, _ = pairs_from_session(load_json(anonymous_path))
        anon_report = build_report(anon_pairs, profile)
        assert anonymous.get("session_id") == appended["session_id"]
        assert anon_report["session_id"] == appended["session_id"]
