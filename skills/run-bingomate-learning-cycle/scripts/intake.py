...
def has_response(value: Any) -> bool:
    ...
    if isinstance(value, dict):
        return any(has_response(item) for item in value.values())
    if isinstance(value, (list, tuple, set)):
        return any(has_response(item) for item in value)
    return True
...
def build_index(pack: dict) -> tuple[dict, dict, list[int], dict, dict]:
    """任务包索引；item_id 是主键，其他标识只作为入口兼容别名。"""
    by_qid: dict[str, int] = {}
    by_loc: dict[str, int] = {}
    seqs: list[int] = []
    spec: dict[int, Any] = {}
    item_id_by_seq: dict[int, str] = {}

    def register(index: dict[str, int], key: str, seq: int, kind: str) -> None:
        if key in index and index[key] != seq:
            raise ValueError(f"任务包里的 {kind} {key!r} 同时指向多个 seq")
        index[key] = seq
    ...
    for item in pack.get("items", []):
        ...
        if seq is None:
            continue
        if seq in spec:
            raise ValueError(f"任务包里的 seq {seq} 重复")
        seqs.append(seq)
        spec[seq] = item.get("acceptable_answers")
        if item.get("item_id"):
            item_id_by_seq[seq] = str(item["item_id"])
        for k in ("item_id", "source_qid", "qid", "id"):
            if item.get(k):
                register(by_qid, str(item[k]), seq, k)
        lk = locator_key(item.get("locator"))
        if lk:
            register(by_loc, lk, seq, "locator")
            # 末段单独也建一份索引，容忍上游少写一级
            tail = lk.split("/")[-1]
            if tail not in by_loc:
                by_loc[tail] = seq
    return by_qid, by_loc, seqs, spec, item_id_by_seq
...
def normalize(pack: dict, raw_log: Any) -> tuple[dict, list[str], list[str]]:
    ...
    by_qid, by_loc, pack_seqs, pack_spec, item_id_by_seq = build_index(pack)
    ...
    for pos, rec in enumerate(records):
        ...
        used.add(seq)
        out["seq"] = seq
        if seq in item_id_by_seq:
            supplied_item_id = rec.get("item_id")
            if supplied_item_id not in (None, item_id_by_seq[seq]):
                issues.append(f"seq {seq} 的 item_id 与任务包不一致")
                continue
            out["item_id"] = item_id_by_seq[seq]
    ...
    answered = {r["seq"] for r in fixed if has_response(r.get("response"))}
...
def main() -> None:
    ...
    try:
        ...
        print(json.dumps({"ok": False, "visibility": "internal", "error": str(exc)}, ensure_ascii=False, indent=2))
        raise SystemExit(2)
