...
任务包的字段命名与画像保持一致；每题额外强制使用会话内唯一 `item_id`。`qid` 与 `locator` 只负责溯源，不再作为作答主键。
...
```json
{
  ...
  "items": [
    ...
    {
      "seq": 3,
      "item_id": "bm-20260806-0193-r1-i3",
      "source": "variant",
      "derived_from": {
        "unit": "Unit 1",
        "period": "第3课时",
    ...
    {
      "seq": 5,
      "item_id": "bm-20260806-0193-r1-i5",
      "source": "workbook",
      "qid": null,
      "locator": {
        "unit": "Unit 1",
  ...
  "data_gaps": [
    "题库无页码字段；精确定位仅供内部回源，对学生只显示单元",
    "非锚题无稳定来源题号，跨会话去重同时保留 item_id 与四元组定位"
  ],

  "out_of_scope": ["层级更新与升降层"]
}
```
...
| `items[].item_id`                                         | `cycle_engine.py append-pack` 校验或自动生成；所有来源都必须有                                                              |
| `next_session.exclude_qids`                                | 画像原有的 `exclude_qids` **加上**本次用掉的题                                                                              |

`next_session.exclude_qids` 在任务包中保留累计值；报告完成后由 `cycle_engine.py` 自动做保序并集合并回画像，不再把回写责任留给外部调用方。

本次用掉的题一律入列，无论做对做错——`exclude_qids` 表示"用过"，不表示"掌握了"。三类的写法：

| 题目 | 写进 `exclude_qids` 的形态 |
| --- | --- |
| 教辅原题 | 四元组串 `Unit 1/第3课时/II/7`；命中锚题时**同时**写锚题 `qid`，两种形态都写，下一轮才能双向对上 |
| 变式题 | 不单独写，写它的 `derived_from` 四元组即可——原题出了候选池，同源变式不会再生 |
| 降级模式的生成题 | 无四元组可写，不写；此时靠考点轮换避免重复，见下一段 |

`append-pack` 默认拒绝再次推送已经排除的教辅原题。只有明确的订正复练可以在该题写 `"repeat_for_correction": true`，并同时写非空 `repeat_reason`；普通复习或候选不足都不能借此绕过去重。

**同一考点连续两轮出现时换外壳。** 画像的 `focus_abilities` 短期内不会变，锚点大概率连着几轮都是同一个，此时保持锚点不动、按 `variant-generation.md` 第三节的「改问法」换题型（括号填空 → 中译英 → 单项选择）。学生认得出是同一个考点，但认不出是同一道题——这正是要的效果。降级模式下没有原题可排除，这条是唯一的防重复手段。
...
| `items[].qid` / `locator` / `derived_from` | **一律 `null`**，不编造课时与题号；`item_id` 仍必须存在                             |
...
精确出处继续保留在结构化任务包与教师明细中。学生和家长只看到单元，不看到课时、练习号或题号；这是演示版统一粒度。

出处怎么说取决于 `items[].source`：

| `source` | 说法 | 依据字段 |
| --- | --- | --- |
| `workbook` | 「这道来自《53同步》Unit 1」 | `locator.unit` |
| `variant` | 「这道按 Unit 1 的同一考点换了个场景」 | `derived_from.unit` + `rule` |
| `generated` | 「这道考的是 by 后面跟动名词」 | `knowledge_point` |

`period`、`exercise_no`、`question_no` 都只供内部回源与教师明细。页码、章节名等题库没有的定位一律不写。

收尾归拢到单元即可：「今天练的是 Unit 1 里 by 表方式这个考点，明天先过一遍」。

**降级模式下整节改写。** `bank_available: false` 时全部题目是 `generated`、`locator` 为 `null`，三种说法只剩第三种，话术里**不出现任何课时与题号**，收尾改说「今天练的是 Unit 1 里 by 表方式这个考点」。各层手册里的话术示例都带着真实题号，降级时不能照抄。

**结构化字段不出现在话术里。** 不说「你的层级是 B」「置信度 medium」「focus_abilities 是语法辨析」，说「这次测得比较浅，先按这个来」「你卡在 by 后面跟什么形式上」。

**层级一律不出口——代号 `A` / `B` / `C` 不说，层级名（拓展挑战 / 巩固提升 / 稳固基础）同样不说。** 第三人称的类别引用也禁止：「考试里 A 层丢分最多的就是这种」「B 层的学生都栽在这儿」不比「你的层级是 B」轻。

分层是给系统排课用的，不是给学生的身份。名字一旦出口，无论说得多像「一条路线」，学生都会立刻推断自己被划进哪一格、上下还有哪几格，剩下的课全在这个念头底下上。**要交代接下来怎么练，就直接讲内容**：

| 不说 | 说 |
| --- | --- |
| 我建议走巩固提升这条路 | 接下来基础题少刷，重点练篇章，一次 6 道左右 |
| 你现在是稳固基础 | 我们慢一点来，先只做基础题，每道我陪你拆开看 |
| 给你按拓展挑战安排 | 基础题只留三成，主攻篇章和中考真题 |

配比、题量、节奏全都可以照实说，它们是安排，不是标签。**这条同样适用于报告环节的收尾话术与家长同步**（见 `report-format.md`）：家长转述一句「老师说你是 C 层」，效果和当面说给孩子一样。
