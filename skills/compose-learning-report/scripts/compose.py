...
def locator_text(loc: Any, bank_available: bool) -> str:
    """出处怎么报。降级模式下一个字都不提课时题号——编一个出来比不给严重得多。"""
    if not bank_available or not isinstance(loc, dict):
        return ""
    parts = [loc.get("unit"), loc.get("period")]
...
def teacher_text(report: dict, profile: dict | None, name: str | None = None) -> str:
    ...
    if fu:
        L.append("【需要跟进】")
        for f in fu:
            loc = locator_text(f.get("locator"), report.get("bank_available") is not False)
    ...
    L.append("【需写回学生档案】本环节只整理，需由系统写入")
    L.append(f"　本次用掉的教辅题：{len(patch.get('exclude_qids') or [])} 道（下次不再重复出）")
...
def main() -> None:
    ...
    if report.get("ok") is False:
        print(json.dumps({"ok": False, "error": f"传进来的是错误信封，不是报告：{report.get('error')}"},
    ...
    files = {
        "student.txt": student_text(report, profile, args.name),
        "parent.txt": parent_text(report, profile, args.name),
        "teacher.txt": teacher_text(report, profile, args.name),
    }
